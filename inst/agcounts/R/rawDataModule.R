...
rawDataModuleServer <- function(id) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    calibratedData <- shiny::reactive({
      shiny::req(input$gt3xFile, input$parser)
      if(input$parser == "agcalibrate") data <- agcounts::agcalibrate(uncalibratedData())
      if(input$parser != "agcalibrate") data <- agcounts::agread(path = input$gt3xFile$datapath, parser = input$parser)
    ...
    processedData <- shiny::reactive({
      ...
      agcounts:::.calculate_raw_metrics(filteredData(), agcounts:::.get_frequency(filteredData()), input$applyEpoch)
    })
    ...
    output$sampleFrequency <- shiny::renderText({
      shiny::req(input$gt3xFile)
      sf <- agcounts:::.get_frequency(uncalibratedData())
